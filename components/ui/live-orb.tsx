...
// LiveOrb by 23rd.dev (shadcn registry @23rd/live-orb), ported from KeepUp's version
// (adds dance / mood / readX). Tailwind classes swapped for `.live-orb*` CSS in
// globals.css (this project has no Tailwind).
...
export function createLiveOrb(
  ...
  const tick = (now: number) => {
    ...
    if (options.dance && !reduce) {
      // Lissajous-style wiggle: left-right on the beat, with a small bounce up and down.
      const t = (now - danceStart) / 1000
      const dx = Math.sin(t * 9) * 0.85
      const dy = 0.12 + Math.sin(t * 18) * 0.28
      look.x += (dx - look.x) * 0.3
      look.y += (dy - look.y) * 0.3
      // Happy blink roughly every half second while dancing.
      if (options.blink && now - lastDanceBlink > 520) {
        blinkAt = now
        lastDanceBlink = now
      }
  ...
  return {
    setOptions(next) {
      if (next.dance && !options.dance) {
        danceStart = performance.now()
        lastDanceBlink = danceStart
      }
...
export function LiveOrb({
  ...
  return (
    <div
      ...
      className={`live-orb ${className ?? ""}`}
      style={{ width: size, height: size }}
    >
      {!hasGl ? (
        <div
          aria-hidden
          className="live-orb__fallback"
          style={fallbackFaceStyle(resolved)}
        >
          <span
            className="live-orb__eye"
            style={{
      ...
      <canvas ref={canvasRef} className="live-orb__canvas" />
    </div>
  )
}
